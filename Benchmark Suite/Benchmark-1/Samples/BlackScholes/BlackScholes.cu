...
int main(int argc, char **argv)
{
    ...
    *d_OptionYears;
    ...
    GPUDevice g0 = GPUDevice(0,"BlackScholesGPU",DIV_UP((OPT_N/2), B_S), B_S);
    g0.startReading();
    for (i = 0; i < NUM_ITERATIONS; i++)
    {
        BlackScholesGPU<<<DIV_UP((OPT_N/2), B_S), B_S/*480, 128*/>>>(
            (float2 *)d_CallResult,
            (float2 *)d_PutResult,
            (float2 *)d_StockPrice,
    ...
    printf("BlackScholes, Throughput = %.4f GOptions/s, Time = %.5f s, Size = %u options, NumDevsUsed = %u, Workgroup = %u\n",
           (((double)(2.0 * OPT_N) * 1.0E-9) / (gpuTime * 1.0E-3)), gpuTime*1e-3, (2 * OPT_N), 1, B_S);
