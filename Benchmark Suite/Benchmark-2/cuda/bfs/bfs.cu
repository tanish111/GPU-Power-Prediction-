...
#include <cuda.h>
#include "GPUDevice.h"
#define MAX_THREADS_PER_BLOCK B_S
...
void BFSGraph( int argc, char** argv) 
{
	...
	do
	{
		...
		stop=false;
		GPUDevice g1 = GPUDevice(0,"Kernel",grid.x*grid.y*grid.z,threads.x*threads.y*threads.z);
		g1.startReading();
		for(int i=0;i<200;i++){
		...
		GPUDevice g2 = GPUDevice(0,"Kernel2",grid.x*grid.y*grid.z,threads.x*threads.y*threads.z);
		g2.startReading();
		for(int i=0;i<200;i++){
